...
int i,d=0, wait_flagg = 1;
char command[20];
bool standby_flag = true;
FILE *fd;
void signal_handler_IO2()
{
	wait_flagg=0;
}
...
int main ()
{
	...
	serial_port = open(PORT, O_RDONLY | O_NOCTTY | O_NONBLOCK);
	if (serial_port<0)
	{
		perror(PORT);
		exit(-1);
...
*/
		...
		if(strcmp(buff,"HS") == 0)
			...
			{
			printf(" Hs Rxd \n");
			serial_data_write("ACK51");
			usleep(50000);
			serial_data_write("CALIB03030");
			serial_data_write("ACK02");
			}
		...
		if(!strcmp(buff, "CM+SA"))
			...
			fclose(fd);
			printf("%s\n", buff);
			//serial_data_write( "SA" + data[2, 9] + ",0,0,0" + "!");
		...
		if(!strcmp(buff, "IH") || (buff, "EH"))
                {
                        if(!strcmp(buff, "EH"))
                        {
                                serial_data_write("ACK67");
                        }
                        else if(!strcmp(buff, "IH"))
                        {
                                serial_data_write("ACK66");
                        }
                        else
                        {
                                ///sdjkcsdkh
