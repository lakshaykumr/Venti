...
float flow_smi()
{
                ...
                double final_flow,Pressure_pascal,Pressure_CMH2O,massFlow,volume_divisor,Flow,Flow_min;
                float volume_timer = 0;
                //float final_flow = 0;
                char data[4] = {0};
...
//                        clock_gettime(CLOCK_MONOTONIC_RAW, &volume_end_time);
  //                      volume_timer = delta_t(&volume_start_time, &volume_end_time,&volume_deltat);
    //                    clock_gettime(CLOCK_MONOTONIC_RAW, &volume_start_time);
			...
			flow_array[AVERAGE_FACTOR - 1] = final_flow;
...
//float smi = flow_smi();
float awm = voltage_flow();

float final_flow =  awm ;
...
//		}
	...
	//		printf("timer = %f  exp_time  == %f \n" , Timer,exp_time);
		...
		{	Ve = volume_final;
			...
			sprintf(d_string, "D@%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f#", round(pressure_insp) , 00,oxygen2_perc_cur ,0 ,0 , 0  ,0 , 100 ,exp_time/1000.00,0);
			printf("%s  \n", d_string);
			printf("data ============== %d \n ", strlen(d_string));
			serial_data_write(d_string);    // Error Code
