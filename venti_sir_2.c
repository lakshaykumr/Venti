...
{	printf(" Pressure Thread_ created \n");
	...
	//	printf(" inside the loop\n");
		...
		while(local_pressure_thread)
			...
			local_pressure_thread = thread_control_pressure;
			//nidhi printf("preesure using thread %f \n",calc_pressure);
                	pthread_mutex_unlock(&mutex);
                	usleep(20000);
        	}
		usleep(1000000);
...
float flow_insp(int address)
{
                ...
                if(read(file, data, 4) != 4)
        {
			...
			for(j=0; j<(AVERAGE_FACTOR-1); j++)
				{
				flow_array[i] =  flow_array[i+1];
				}
			flow_array[AVERAGE_FACTOR - 1] = final_flow;
			for(j=0;j<AVERAGE_FACTOR; j++)
				{
				printf("new flow array => %f", flow_array[j]);
				}
			}

			if (flow_flag== false)
			{
			flow_array[i] = final_flow;
			i++;
			average_count++;
			}
...
//////////////////////////////////////////////////////////////////////////////////
	...
	int sum_flow=0;
	float final_flow_averaging;
	while(1)
	{
		read_settings();
		for(int i=0 ; i<20 ;i++)
		 {final_flow_averaging=flow_insp(0x28);
		   sum_flow = sum_flow + final_flow_averaging ;
		   final_flow = sum_flow/20  ;
		  }
		  sum_flow =0;     //nidhi
          	 printf("Flow =%f\n" ,  final_flow);    //nidhi
		current_mode = mode_change();
		printf( " mode == %d \n",current_mode);
       		//f( current_mode/10 != mode_para/10)
        	//{
...
//		printf(" Pressure == %f Time == %d    %f\n" , pressure_insp ,exp_time,duty_cycle_\valve_cal);
		switch(current_mode%10)
		{
			...
			case 2 : trigger_flag = true;cmv = false; psv_flag =false ;pc_ac_flag = false;printf("simv enabled\n");break;		// psv_flag indicate that every trigger breath is on Pinsp
	...
	//	flow_combine = flow_insp();
		...
		printf("%d     %f  \n  ",trigger_flag,final_setting.Trigger);
...
//
		...
		if(pressure_insp <= 5) //LAKSHAY
				{
				//printf("+++++++++++++++++++++++breaking----------------");
			//nidhi	printf("valve close @ Pressure => %f \n", pressure_insp);
